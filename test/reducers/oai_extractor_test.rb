...
class OaiExtractorTest < Minitest::Test
  ...
  let(:response) do
    VCR.use_cassette("oai_first_page") do
      Letl::OaiExtractor.new(args: {base_uri: 'https://server16022.contentdm.oclc.org/oai.php'})
    end
  end
  ...
  def test_resumption_token
    VCR.use_cassette("oai_second_page") do
      response = Letl::OaiExtractor.new(args: {base_uri: 'https://server16022.contentdm.oclc.org/oai.php', resumption_token: 'swede:96:oclc-cdm-allsets:0000-00-00:9999-99-99:oai_dc'})
  ...
  def test_last_batch_stop_true
    ...
    expected = {:data=>nil, :reducers=>[{:reducer=> Letl::OaiExtractor, :args=>{:base_uri=>"https://example.com", :resumption_token=>{}}}], :stop=>true}
    ...
    response = Letl::OaiExtractor.new(args: {base_uri: 'https://example.com'}, oai_client: mock_oai_client)
